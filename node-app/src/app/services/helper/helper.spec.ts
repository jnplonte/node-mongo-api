...
describe('helper service', () => {
	...
	it('should check the password', (done) => {
		expect(services.getPassword('xxxxxx', 'saltbae')).to.equal(
			'f0554e1d6fd6b4aa2a78a37f2299b27c605884b8335162ac66be37fa5b878e77'
		);
	...
	it('should read and create jwt token', (done) => {
		...
		expect(token).to.equal(
			'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6InV1aWQtaWQifQ.aWM9UsKSx1ul4-KK2MmUK8-FwVs6bJDfFwFG-M3rKz8'
		);
	...
	it('should get default configs', (done) => {
		...
		expect(services.defaultUserRole).to.be.a('string');
