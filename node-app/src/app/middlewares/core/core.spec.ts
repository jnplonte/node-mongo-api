...
describe('core middleware', () => {
	...
	it('should get autofill value', (done) => {
		...
		expect(middlewares.autoFillPostInformation({ dataOne: 'test' }, { id: '1' })).to.include({ dataOne: 'test' });

		expect(middlewares.autoFillPutInformation({ dataOne: 'test' }, {})).to.include({ dataOne: 'test' });
		expect(middlewares.autoFillPutInformation({ dataOne: 'test' }, { id: '1' })).to.include({ dataOne: 'test' });
